...
let count ?skip ?limit ?(query=Bson.empty) m =
  ...
  let c_bson =
    match limit with
      | Some n -> Bson.add_element "limit" (Bson.create_int n) c_bson
      | None -> c_bson
  in
  let c_bson =
    match skip with
      | Some n -> Bson.add_element "skip" (Bson.create_int n) c_bson
      | None -> c_bson
  in
...
let drop_database m =
  let m = change_collection m "$cmd" in
  find_q_one m (Bson.add_element "dropDatabase" (Bson.create_int 1) Bson.empty)
...
let ensure_index m key_bson options =
  let default_name () =
    ...
    List.fold_left (
      fun s (k,e) ->
        let i = Bson.get_int e in
        if s = "" then
          Printf.sprintf "%s_%d" k i
        else
          Printf.sprintf "%s_%s_%d" s k i
      ...
      fun acc o ->
        match o with
          ...
          | ExpireAfterSeconds i ->
            Bson.add_element "expireAfterSeconds" (Bson.create_int i) acc
          | V i ->
            if i <> 0 && i <> 1 then raise (Mongo_failed "Version number for index must be 0 or 1");
            has_version := true;
            Bson.add_element "v" (Bson.create_int i) acc
          | Weight bson ->
  ...
  let main_bson =
    if !has_version = false then
      Bson.add_element "v" (Bson.create_int 1) main_bson
    else main_bson
  in
...
let ensure_simple_index ?(options=[]) m field =
  let key_bson = Bson.add_element "key" (Bson.create_doc_element (Bson.add_element field (Bson.create_int 1) Bson.empty)) Bson.empty in
...
let ensure_multi_simple_index ?(options=[]) m fields =
  let key_bson =
    List.fold_left (
      fun acc f ->
        Bson.add_element f (Bson.create_int 1) acc
    ) Bson.empty fields
  in
