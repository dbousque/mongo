...
let rec of_yojson = function
  ...
  | `List l -> Array (List.map of_yojson l)
  | `Tuple l -> Array (List.map of_yojson l)
  | `Assoc l -> Doc (yojson_assoc_to_bson l)
  | `Variant (k, None) -> String k
  | `Variant (k, Some v) -> Array [String k ; of_yojson v]

and yojson_assoc_to_bson yojson =
  let rec _yojson_assoc_to_bson doc = function
    | [] -> doc
    | (key, value)::rest -> ( let new_doc = add_element key (of_yojson value) doc in
...
let create_doc_element v = Doc v;;
...
let get_doc_element = function | Doc v -> v | _ -> raise Wrong_bson_type;;
...
let get_boolean = function | Bool v -> v | _ -> raise Wrong_bson_type;;
...
let get_int = function | Int v -> v | _ -> raise Wrong_bson_type;;
...
let encode doc =
  ...
  encode_int32 buf (Int32.of_int v)
      | Timestamp v ->
...
let decode_boolean str cur = (Bool (if str.[cur] = '\x00' then false else true), cur+1);;
